#ifndef _IMAGE_DATA_READER_HPP_
#define _IMAGE_DATA_READER_HPP_
...
class ImageDataReader {
 public:
  explicit ImageDataReader(const LayerParameter& param);
  ~ImageDataReader();

  inline BlockingQueue<AnnotatedDatum*>& free() const {
    return queue_pair_->free_;
  }
  inline BlockingQueue<AnnotatedDatum*>& full() const {
 ...
 protected:
  ...
  class QueuePair {
   public:
    ...
    BlockingQueue<AnnotatedDatum*> free_;
    BlockingQueue<AnnotatedDatum*> full_;
  ...
  static inline string source_key(const LayerParameter& param) {
    return param.name() + ":" + param.annotated_data_param().source();
  }
