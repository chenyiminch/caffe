...
def CreateAnnotatedDataLayer(source, root_folder, batch_size=32, 
...
def VGGNetBody_quarter(net, from_layer, need_fc=True, fully_conv=False, reduced=False,
    ...
    net.conv1_1 = L.Convolution(net[from_layer], num_output=16, pad=1, kernel_size=3, **kwargs)

    net.relu1_1 = L.ReLU(net.conv1_1, in_place=True)
    net.conv1_2 = L.Convolution(net.relu1_1, num_output=16, pad=1, kernel_size=3, **kwargs)
    net.relu1_2 = L.ReLU(net.conv1_2, in_place=True)

    if nopool:
        name = 'conv1_3'
        net[name] = L.Convolution(net.relu1_2, num_output=16, pad=1, kernel_size=3, stride=2, **kwargs)
    else:
        name = 'pool1'
    ...
    if nopool:
        name = 'conv2_3'
        net[name] = L.Convolution(net.relu2_2, num_output=32, pad=1, kernel_size=3, stride=2, **kwargs)
    else:
        name = 'pool2'
    ...
    if need_fc:
        ...
        if fully_conv:
            if dilated:
                if reduced:
                    net.fc6 = L.Convolution(net[name], num_output=256, pad=6, kernel_size=3, dilation=6, **kwargs)
                else:
                    net.fc6 = L.Convolution(net[name], num_output=1024, pad=6, kernel_size=7, dilation=2, **kwargs)
            else:
                if reduced:
                    net.fc6 = L.Convolution(net[name], num_output=256, pad=3, kernel_size=3, dilation=3, **kwargs)
                else:
                    net.fc6 = L.Convolution(net[name], num_output=1024, pad=3, kernel_size=7, **kwargs)
